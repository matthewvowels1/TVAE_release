...
class Model(PyroModule):
	...
	def forward(self, x, t=None, y=None, size=None):
		...
		with pyro.plate("data", size, subsample=x):
			...
			x2_cats = x_cats[:, 1]
			x3_cats = x_cats[:, 2]
			print(x[:, self.all_indices[0]].min(), x[:, self.all_indices[0]].max())
			x_P = pyro.sample("x_P", self.x_dist_P(zo, zc, zt, zy), obs=x[:, self.all_indices[0]]+1)
			x_B = pyro.sample("x_B", self.x_dist_B(zo, zc, zt, zy), obs=x[:, self.all_indices[1]])
			x_O1 = pyro.sample("x_O1", self.x_dist_O1(zo, zc, zt, zy), obs=x1_cats)
			x_O2 = pyro.sample("x_O2", self.x_dist_O2(zo, zc, zt, zy), obs=x2_cats)
			x_O3 = pyro.sample("x_O3", self.x_dist_O3(zo, zc, zt, zy), obs=x3_cats)
			x_N = pyro.sample("x_N", self.x_dist_N(zo, zc, zt, zy), obs=x[:, self.all_indices[3]])
			print(x[:, self.all_indices[4]].shape, self.x_dist_S(zo, zc, zt, zy).event_shape, self.x_dist_S(zo, zc, zt, zy).batch_shape)
			...
			x_E = pyro.sample("x_E", self.x_dist_E(zo, zc, zt, zy), obs=x[:, self.all_indices[6]])
